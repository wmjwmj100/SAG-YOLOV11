...
class BaseSolution:
    ...
    Methods:
        adjust_box_label: Generate formatted labels for bounding box.
    ...
    def adjust_box_label(self, cls: int, conf: float, track_id: Optional[int] = None) -> Optional[str]:
        """
        Generate a formatted labels for a bounding box.

        This method constructs a labels string for a bounding box using the class index and confidence score.
        Optionally includes the track ID if provided. The labels format adapts based on the display settings
        ...
        Returns:
            (str | None): The formatted labels string if `self.show_labels` is True; otherwise, None.
        """
...
class SolutionAnnotator(Annotator):
    ...
    Methods:
        ...
        sweep_annotator: Visualize a vertical sweep line and optional labels.
        visioneye: Map and connect object centroids to a visual "eye" point.
        circle_label: Draw a circular labels within a bounding box.
        text_label: Draw a rectangular labels within a bounding box.
    ...
    def sweep_annotator(
        ...
        """
        Draw a sweep annotation line and an optional labels.

        Args:
            line_x (int): The x-coordinate of the sweep line.
            line_y (int): The y-coordinate limit of the sweep line.
            label (str, optional): Text labels to be drawn in center of sweep line. If None, no labels is drawn.
            color (Tuple[int, int, int]): RGB color for the line and labels background.
            txt_color (Tuple[int, int, int]): RGB color for the labels text.
        """
        # Draw the sweep line
    ...
    def circle_label(
        ...
        """
        Draw a labels with a background circle centered within a given bounding box.
        ...
        """
        if len(label) > 3:
            LOGGER.warning(f"Length of labels is {len(label)}, only first 3 letters will be used for circle annotation.")
            label = label[:3]
    ...
    def text_label(
        ...
        """
        Draw a labels with a background rectangle centered within a given bounding box.
