...
from .modules.mobilenet import MobileNetV3
import torch
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        Args:
            batch (dict): Dictionary containing image and labels data.
...
def change_channels(x):
    # 这个函数没用什么用，只是辅助加注释用的。这种架构改进改进好之后益处很大，有兴趣可以找我qq3525983226
    # 暑期老师有一个钢铁项目，有公司的数据集，有兴趣也可以找我
    return 0
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            c2 = ch[f]
        print(f"Creating module  with args: {args}")
        m_ = torch.nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module

        t = str(m)[8:-2].replace("__main__.", "")  # module type
        print(f"Creating module {t} with args: {args}")
