...
def smooth_bce(eps: float = 0.1) -> Tuple[float, float]:
    ...
    Args:
        eps (float, optional): The epsilon value for labels smoothing.

    Returns:
        pos (float): Positive labels smoothing BCE target.
        neg (float): Negative labels smoothing BCE target.
...
def plot_mc_curve(
    ...
    Args:
        ...
        save_dir (Path, optional): Path to save the plot.
        names (Dict[int, str], optional): Dictionary mapping class indices to class names.
        xlabel (str, optional): X-axis labels.
        ylabel (str, optional): Y-axis labels.
        on_plot (callable, optional): Function to call after plot is saved.
    """
    import matplotlib.pyplot as plt  # scope for faster 'import ultralytics'
