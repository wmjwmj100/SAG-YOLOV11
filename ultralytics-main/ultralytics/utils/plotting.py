...
class Annotator:
    ...
    def box_label(self, box, label: str = "", color: tuple = (128, 128, 128), txt_color: tuple = (255, 255, 255)):
        """
        Draw a bounding box on an image with a given labels.

        Args:
            box (tuple): The bounding box coordinates (x1, y1, x2, y2).
            label (str, optional): The text labels to be displayed.
        ...
        Examples:
            ...
            >>> im0 = cv2.imread("test.png")
            >>> annotator = Annotator(im0, line_width=10)
            >>> annotator.box_label(box=[10, 20, 30, 40], labels="person")
        """
        ...
        if self.pil:
            ...
            if label:
                ...
                if p1[0] > self.im.size[0] - w:  # size is (w, h), check if labels extend beyond right side of image
            ...
            if label:
                ...
                outside = p1[1] >= h  # labels fits outside box
                if p1[0] > self.im.shape[1] - w:  # shape is (h, w), check if labels extend beyond right side of image
