...
def _scale_bounding_box_to_original_image_shape(
    ...
    YOLO resizes images during training and the labels values are normalized based on this resized shape.
...
def _format_ground_truth_annotations_for_detection(img_idx, image_path, batch, class_name_map=None) -> Optional[dict]:
    ...
    Returns:
        (dict | None): Formatted ground truth annotations with the following structure:
            ...
            - 'labels': Label string with format "gt_{class_name}"
...
def _format_prediction_annotations(image_path, metadata, class_label_map=None, class_map=None) -> Optional[dict]:
    ...
    Args:
        ...
        class_map (dict, optional): Additional class mapping for labels conversion.
    ...
    for prediction in predictions:
        ...
        annotation_data = {"boxes": [boxes], "labels": cls_label, "score": score}
...
def _fetch_annotations(
    ...
    Args:
        ...
        class_map (dict): Additional class mapping for labels conversion.
...
def _log_plots(experiment, trainer) -> None:
    """
    Log evaluation plots and labels plots for the experiment.
